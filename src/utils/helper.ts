...
export function requestLogin(vue) {
  ...
  let path = `https://mixin-oauth.fox.one/?client_id=${CONFIG.MIXIN_CLIENT_ID}&scope=PROFILE:READ+ASSETS:READ+SNAPSHOTS:READ&response_type=code`;
...
export function risk(currentRatio, minimumRatio) {
  ...
  if (!number.isValid(risk)) return "riskGrey";
  if (risk < 0) {
    return "riskGrey";
  } else if (risk <= 1.25) {
    return "riskRed";
